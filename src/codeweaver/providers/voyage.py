...
class VoyageAIProvider(CombinedProvider):
    ...
    def _validate_config(self) -> None:
        ...
        if embedding_model not in self._capabilities.supported_embedding_models:
            ...
            raise ValueError(f'Unknown VoyageAI embedding model: {embedding_model}. Available: {available}')
        ...
        if rerank_model not in self._capabilities.supported_reranking_models:
            ...
            raise ValueError(f'Unknown VoyageAI reranking model: {rerank_model}. Available: {available}')
    ...
    async def embed_documents(self, texts: list[str], context: dict[str, Any] | None=None) -> list[list[float]]:
        ...
        if cache_service:
            cache_key = {'provider': 'voyage_ai', 'model': self._embedding_model, 'texts': texts, 'input_type': 'document', 'dimension': self._dimension}
        ...
        try:
            result = self.client.embed(texts=texts, model=self._embedding_model, input_type='document', output_dimension=self._dimension)
    ...
    async def embed_query(self, text: str, context: dict[str, Any] | None=None) -> list[float]:
        ...
        cache_service = context.get('caching_service')
        if cache_service:
            cache_key = {'provider': 'voyage_ai', 'model': self._embedding_model, 'text': text, 'input_type': 'query', 'dimension': self._dimension}
            cached_result = await cache_service.get(cache_key)
            if cached_result:
        ...
        if rate_limiter:
            ...
            await self._rate_limit()
        try:
            result = self.client.embed(texts=[text], model=self._embedding_model, input_type='query', output_dimension=self._dimension)
    ...
    async def rerank(self, query: str, documents: list[str], top_k: int | None=None) -> list[RerankResult]:
        ...
        try:
            ...
            result = self.client.rerank(query=query, documents=documents, model=self._rerank_model, top_k=top_k)
            rerank_results = [RerankResult(index=item.index, relevance_score=item.relevance_score, document=item.document if hasattr(item, 'document') else None) for item in result.results]
        except Exception:
    ...
    def get_provider_info(self) -> EmbeddingProviderInfo:
        ...
        return EmbeddingProviderInfo(name=ProviderType.VOYAGE_AI.value, display_name=registry_entry.display_name, description=registry_entry.description, supported_capabilities=[ProviderCapability.EMBEDDING, ProviderCapability.RERANKING, ProviderCapability.BATCH_PROCESSING, ProviderCapability.CUSTOM_DIMENSIONS], capabilities=capabilities, default_models={'embedding': capabilities.default_embedding_model, 'reranking': capabilities.default_reranking_model}, supported_models={'embedding': capabilities.supported_embedding_models, 'reranking': capabilities.supported_reranking_models}, rate_limits={'requests_per_minute': capabilities.requests_per_minute, 'tokens_per_minute': capabilities.tokens_per_minute}, requires_api_key=capabilities.requires_api_key, max_batch_size=capabilities.max_batch_size, max_input_length=capabilities.max_input_length, native_dimensions=capabilities.native_dimensions)
