...
class CodeWeaverFactory:
    ...
    def __init__(self, config: 'CodeWeaverConfig | None' = None, registry: ComponentRegistry | None = None, services_manager: 'ServicesManager | None' = None):
    ...
    def _initialize_builtin_backends(self) -> None:
        ...
        try:
            ...
            backend_info = BackendInfo(name='qdrant', backend_type='qdrant', description='Qdrant vector database backend', connection_requirements={'url': 'Qdrant server URL'}, optional_parameters={'api_key': 'Optional API key for authentication'})
            capabilities = BackendCapabilities(supports_filtering=True, supports_metadata=True, supports_batch_operations=True, max_vector_dimension=65536)
            self.registry.register_backend(name='qdrant', backend_class=QdrantBackend, capabilities=capabilities, backend_info=backend_info)
        except ImportError:
    ...
    def _initialize_builtin_sources(self) -> None:
        ...
        try:
            ...
            source_info = SourceInfo(name='filesystem', source_type='filesystem', description='Local filesystem data source', supported_schemes=['file'], configuration_schema={'root_path': 'string'})
            capabilities = SourceCapabilities(supports_streaming=True, supports_filtering=True, supports_metadata=True, supports_incremental_updates=False)
            self.registry.register_source(name='filesystem', source_class=FileSystemSource, capabilities=capabilities, source_info=source_info)
        except ImportError:
    ...
    def _initialize_builtin_services(self) -> None:
        ...
        try:
            ...
            self.registry.register_service_provider(service_type=ServiceType.CHUNKING, provider_name='default', provider_class=ChunkingService, capabilities=ServiceCapabilities(supports_streaming=True, supports_batching=True, max_batch_size=100))
            self.registry.register_service_provider(service_type=ServiceType.FILE_FILTERING, provider_name='default', provider_class=FileFilteringService, capabilities=ServiceCapabilities(supports_streaming=True, supports_batching=False))
            self.registry.register_service_provider(service_type=ServiceType.RATE_LIMITING, provider_name='default', provider_class=RateLimitingService, capabilities=ServiceCapabilities(supports_streaming=False, supports_batching=True, max_batch_size=1000, supports_async=True))
            self.registry.register_service_provider(service_type=ServiceType.CACHING, provider_name='default', provider_class=CachingService, capabilities=ServiceCapabilities(supports_streaming=False, supports_batching=True, max_batch_size=1000, supports_async=True))
        except ImportError:
    ...
    async def create_server(self, config: 'CodeWeaverConfig | None' = None, *, validate_config: bool = True) -> 'CodeWeaverServer':
    ...
    def get_available_components(self) -> dict[str, dict[str, Any]]:
        ...
        """
        return {'backends': self.registry.list_backends(), 'sources': self.registry.list_sources(), 'services': {service_type.value: self.registry.list_service_providers(service_type) for service_type in ServiceType}, 'providers': self._provider_factory.get_available_embedding_providers()}
    ...
    async def _create_service_safely(self, service_type: ServiceType, service_key: str) -> Any | None:
    ...
    async def _get_services_context(self) -> dict[str, Any]:
        ...
        try:
            ...
            service_definitions = [(ServiceType.RATE_LIMITING, 'rate_limiting_service'), (ServiceType.CACHING, 'caching_service'), (ServiceType.CHUNKING, 'chunking_service'), (ServiceType.FILE_FILTERING, 'filtering_service')]
