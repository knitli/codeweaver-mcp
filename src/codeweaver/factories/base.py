...
class BaseRegistry(ABC):
    ...
    def register_component(self, name: str, factory: ComponentFactory) -> None:
        ...
        if name in self._factories:
            self.logger.warning(f"Overriding existing {self.component_type.value} factory: {name}")
        
        self._factories[name] = factory
        self.logger.debug(f"Registered {self.component_type.value} factory: {name}")
    ...
    def clear_registry(self) -> None:
        ...
        self.logger.debug(f"Cleared {self.component_type.value} registry")
