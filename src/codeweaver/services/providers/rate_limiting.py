...
class RateLimitingService(BaseServiceProvider):
    ...
    def _create_bucket(self, key: str, requests_per_second: float, burst_capacity: int) -> None:
        ...
        self._buckets[key] = TokenBucket(capacity=burst_capacity, tokens=float(burst_capacity), refill_rate=requests_per_second)
    ...
    async def acquire(self, provider: str, tokens: int=1) -> None:
        ...
        async with self._locks[bucket_key]:
            ...
            if wait_time > 0:
                ...
                logger.debug('Rate limiting %s: waiting %ss for %s tokens', provider, wait_time, tokens)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'total_requests': self._total_requests, 'blocked_requests': self._blocked_requests, 'block_rate': self._blocked_requests / max(1, self._total_requests), 'total_wait_time': self._total_wait_time, 'average_wait_time': self._total_wait_time / max(1, self._blocked_requests), 'active_buckets': list(self._buckets.keys())}
    ...
    async def health_check(self) -> ServiceHealth:
        ...
        try:
            ...
            if bucket_count == 0:
                return ServiceHealth(status=HealthStatus.UNHEALTHY, message='No rate limiting buckets configured')
            depleted_buckets = []
            ...
            if len(depleted_buckets) > len(self._buckets) * 0.5:
                return ServiceHealth(status=HealthStatus.DEGRADED, message=f'Many buckets depleted: {depleted_buckets}')
            return ServiceHealth(status=HealthStatus.HEALTHY, message=f'Rate limiting active with {bucket_count} buckets')
        except Exception as e:
            return ServiceHealth(status=HealthStatus.UNHEALTHY, message=f'Rate limiting health check failed: {e}')

    def get_capabilities(self) -> ServiceCapabilities:
        """Get service capabilities."""
        return ServiceCapabilities(supports_streaming=False, supports_batching=True, max_batch_size=1000, supports_async=True)
