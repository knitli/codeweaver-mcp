...
class RateLimitingService(BaseServiceProvider):
    ...
    def __init__(self, config: RateLimitConfig | None = None):
        ...
        # Token buckets for different providers/endpoints
    ...
    async def initialize(self) -> None:
        ...
        # Create provider-specific buckets
    ...
    async def acquire(self, provider: str, tokens: int = 1) -> None:
        ...
        async with self._locks[bucket_key]:
            ...
            if wait_time > 0:
                ...
                logger.debug(f"Rate limiting {provider}: waiting {wait_time:.2f}s for {tokens} tokens")
                await asyncio.sleep(wait_time)
                
                # Try again after waiting
                if not bucket.consume(tokens):
                    logger.warning(f"Failed to acquire tokens for {provider} after waiting")
                    raise asyncio.TimeoutError(f"Rate limit exceeded for {provider}")
    ...
    async def health_check(self) -> ServiceHealth:
        ...
        try:
            ...
            # Check if any buckets are severely depleted
            depleted_buckets = []
            for key, bucket in self._buckets.items():
                bucket._refill()
                if bucket.tokens < bucket.capacity * 0.1:  # Less than 10% capacity
