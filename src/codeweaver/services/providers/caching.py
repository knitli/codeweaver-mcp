...
class CachingService(BaseServiceProvider):
    ...
    async def set(self, key: Any, value: Any, ttl: int | None=None) -> None:
        ...
        async with self._lock:
            ...
            entry = CacheEntry(value=value, created_at=time.time(), ttl=effective_ttl, size_bytes=size_bytes)
    ...
    async def _ensure_capacity(self, new_size: int) -> None:
        ...
        while len(self._cache) >= self.config.max_size or self._total_size_bytes + new_size > max_size_bytes:
            if not self._cache:
                break
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {'hits': self._hits, 'misses': self._misses, 'hit_rate': hit_rate, 'evictions': self._evictions, 'total_entries': len(self._cache), 'total_size_bytes': self._total_size_bytes, 'total_size_mb': self._total_size_bytes / (1024 * 1024), 'max_size': self.config.max_size, 'max_memory_mb': self.config.max_memory_mb}
    ...
    async def health_check(self) -> ServiceHealth:
        ...
        try:
            ...
            if memory_usage_pct > 95:
                return ServiceHealth(status=HealthStatus.DEGRADED, message=f'High memory usage: {memory_usage_pct:.1f}%')
            ...
            return ServiceHealth(status=HealthStatus.HEALTHY, message=f"Cache active: {stats['total_entries']} entries, {stats['hit_rate']:.2f} hit rate")
        except Exception as e:
            return ServiceHealth(status=HealthStatus.UNHEALTHY, message=f'Cache health check failed: {e}')

    def get_capabilities(self) -> ServiceCapabilities:
        """Get service capabilities."""
        return ServiceCapabilities(supports_streaming=False, supports_batching=True, max_batch_size=self.config.max_size, supports_async=True)
