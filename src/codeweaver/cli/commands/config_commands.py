...
async def generate(
    ...
    template: Annotated[
        str, Parameter(name=["--template", "-t"], help="Configuration template to use")
    ] = "default",
    fmt: Annotated[
        OutputFormat, Parameter(name=["--format", "-f"], alias="format", help="Output format for results")
    ] = OutputFormat.TEXT,
    overwrite: Annotated[
    ...
    try:
        ...
        if output_path.exists() and (not overwrite):
            raise_cli_error(
                f"Configuration file already exists: {output_path}. Use --overwrite to replace it.")
...
async def validate(
    ...
    try:
        ...
        if not config_file.exists():
            raise_cli_error(f"Configuration file not found: {config_file}.")
        try:
...
async def mcp_insert(
    ...
    server_name: Annotated[
        str, Parameter(name=["--name", "-n"], help="Name for the MCP server entry")
    ] = "codeweaver",
    server_path: Annotated[
        str | None, Parameter(name=["--path", "-p"], help="Path to CodeWeaver server executable")
    ] = None,
    args: Annotated[
        list[str] | None, Parameter(name=["--args", "-a"], help="Command line arguments for server")
    ] = None,
    env_vars: Annotated[
        list[str] | None, Parameter(name=["--env", "-e"], help="Environment variables (KEY=VALUE format)")
    ] = None,
    fmt: Annotated[
        OutputFormat, Parameter(name=["--format", "-f"], alias="format", help="Output format for results")
    ] = OutputFormat.TEXT,
    backup: Annotated[
        bool, Parameter(name=["--backup", "-b"], help="Create backup of existing configuration")
    ] = True,
    force: Annotated[
...
async def mcp_remove(
    ...
    server_name: Annotated[
        str, Parameter(name=["--name", "-n"], help="Name of MCP server entry to remove")
    ] = "codeweaver",
    fmt: Annotated[
        OutputFormat, Parameter(name=["--format", "-f"], alias="format", help="Output format for results")
    ] = OutputFormat.TEXT,
    backup: Annotated[
        bool, Parameter(name=["--backup", "-b"], help="Create backup before removal")
    ] = True,
) -> None:
...
async def mcp_list(
    ...
    show_all: Annotated[
        bool,
        Parameter(name=["--all", "-a"], help="Show all MCP clients, even if config files don't exist"),
    ] = False,
) -> None:
