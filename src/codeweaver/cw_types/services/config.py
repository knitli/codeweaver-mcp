...
from codeweaver.cw_types.enums import ChunkingStrategy, PerformanceMode
...
class ImplicitLearningServiceConfig(ServiceConfig):
    ...
    satisfaction_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Satisfaction threshold")
    ] = 0.8
    learning_rate: Annotated[
        float, Field(gt=0.0, le=1.0, description="Learning rate for pattern updates")
    ] = 0.1
    pattern_retention_days: Annotated[
        int, Field(gt=0, description="Days to retain behavioral patterns")
    ] = 30
    min_interactions_for_pattern: Annotated[
        int, Field(gt=0, description="Min interactions to identify pattern")
    ] = 5

    # Behavioral signal weights
    response_time_weight: Annotated[
        float, Field(ge=0.0, le=1.0, description="Response time signal weight")
    ] = 0.3
    follow_up_pattern_weight: Annotated[
        float, Field(ge=0.0, le=1.0, description="Follow-up pattern weight")
    ] = 0.4
    session_completion_weight: Annotated[
        float, Field(ge=0.0, le=1.0, description="Session completion weight")
    ] = 0.3

    # Learning optimization
    enable_pattern_clustering: Annotated[bool, Field(description="Enable pattern clustering")] = (
        True
    )
    max_patterns_per_type: Annotated[int, Field(gt=0, description="Max patterns per type")] = 1000
    pattern_confidence_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Pattern confidence threshold")
    ] = 0.7
...
class ContextIntelligenceServiceConfig(ServiceConfig):
    ...
    llm_identification_enabled: Annotated[bool, Field(description="Enable LLM identification")] = (
        True
    )
    behavioral_fingerprinting: Annotated[
        bool, Field(description="Enable behavioral fingerprinting")
    ] = True
    privacy_mode: Annotated[str, Field(description="Privacy mode for data collection")] = (
        "hash_identifiers"
    )

    # Context analysis settings
    context_adequacy_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Context adequacy threshold")
    ] = 0.7
    min_context_elements: Annotated[
        int, Field(gt=0, description="Minimum context elements required")
    ] = 3
    context_richness_scoring: Annotated[
        bool, Field(description="Enable context richness scoring")
    ] = True

    # LLM profiling
    enable_model_detection: Annotated[bool, Field(description="Enable LLM model detection")] = True
    model_confidence_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Model detection confidence")
    ] = 0.6
    behavioral_feature_extraction: Annotated[
        bool, Field(description="Extract behavioral features")
    ] = True

    # Session tracking
    session_timeout: Annotated[float, Field(gt=0, description="Session timeout in seconds")] = (
        1800.0
    )
...
class ZeroShotOptimizationServiceConfig(ServiceConfig):
    ...
    success_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Success probability threshold")
    ] = 0.8
    optimization_aggressiveness: Annotated[str, Field(description="Optimization strategy")] = (
        "balanced"  # conservative, balanced, aggressive
    )

    # Success prediction
    enable_success_prediction: Annotated[bool, Field(description="Enable success prediction")] = (
        True
    )
    prediction_confidence_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Prediction confidence threshold")
    ] = 0.8
    historical_success_weight: Annotated[
        float, Field(ge=0.0, le=1.0, description="Weight for historical success")
    ] = 0.4
    context_adequacy_weight: Annotated[
        float, Field(ge=0.0, le=1.0, description="Weight for context adequacy")
    ] = 0.6

    # Optimization strategies
    enable_context_enrichment: Annotated[bool, Field(description="Enable context enrichment")] = (
        True
    )
    enable_strategy_optimization: Annotated[
        bool, Field(description="Enable strategy optimization")
    ] = True
    max_optimization_attempts: Annotated[
        int, Field(gt=0, description="Max optimization attempts")
    ] = 3

    # Pattern database
    success_pattern_retention: Annotated[
        int, Field(gt=0, description="Success pattern retention days")
    ] = 90
    min_pattern_frequency: Annotated[
        int, Field(gt=0, description="Min frequency for pattern storage")
    ] = 3
    pattern_similarity_threshold: Annotated[
        float, Field(ge=0.0, le=1.0, description="Pattern similarity threshold")
    ] = 0.8
...
class ServicesConfig(BaseModel):
    ...
    implicit_learning: Annotated[
        ImplicitLearningServiceConfig, Field(description="Implicit learning config")
    ] = Field(default_factory=ImplicitLearningServiceConfig)
    context_intelligence: Annotated[
        ContextIntelligenceServiceConfig, Field(description="Context intelligence config")
    ] = Field(default_factory=ContextIntelligenceServiceConfig)
    zero_shot_optimization: Annotated[
        ZeroShotOptimizationServiceConfig, Field(description="Zero-shot optimization config")
    ] = Field(default_factory=ZeroShotOptimizationServiceConfig)
