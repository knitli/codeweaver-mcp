...
class EmbeddingProviderInfo:
    ...
    default_models: "dict[str, str | type[ModelCapabilityEnum]] | None" = (
        None  # capability -> default model
    )
    supported_models: "dict[str, list[str | type[ModelCapabilityEnum]]] | None" = (
        None  # capability -> list of models
    )
    rate_limits: dict[str, int] | None = None  # operation -> limit per minute
    requires_api_key: bool = True
    max_batch_size: int | None = None
    max_input_length: int | None = None
    native_dimensions: "dict[str | type[ModelCapabilityEnum], int] | None" = (
        None  # model -> native dimensions
    )
    native_context_length: "dict[str | type[ModelCapabilityEnum], int] | None" = (
        None  # model -> native context length
    )
...
def _create_provider_registry() -> dict[ProviderType, ProviderRegistryEntry]:
    ...
    return {
        ProviderType.VOYAGE_AI: ProviderRegistryEntry(
            ...
            supported_models={
                ModelFamily.CODE_EMBEDDING: [
                    VoyageModel.CODE_3,
                    VoyageModel.CONTEXT_3,
                    VoyageModel.VOYAGE_3_LARGE,
                    VoyageModel.VOYAGE_35,
                    VoyageModel.VOYAGE_35_LITE,
                ],
                ModelFamily.TEXT_EMBEDDING: [
                    member for member in VoyageModel.members() if "code" not in member.value
                ],
        ...
        ProviderType.OPENAI: ProviderRegistryEntry(
            ...
            supported_models={ModelFamily.TEXT_EMBEDDING: list(OpenAIModel.members())},
        ),
        ...
        ProviderType.SENTENCE_TRANSFORMERS: ProviderRegistryEntry(
            ...
            supported_models={ModelFamily.TEXT_EMBEDDING: list(SentenceTransformerModel.members())},
        ),
