...
from codeweaver.types.providers.enums import ModelCapabilityEnum
...
class ProviderCapabilities(BaseModel):
    ...
    supports_nlp: bool = Field(False, description="Supports natural language processing")
    ...
    # Model configuration
    default_embedding_model: type[ModelCapabilityEnum] | str | None = Field(None, description="Default embedding model")
    default_reranking_model: type[ModelCapabilityEnum] | str | None = Field(None, description="Default reranking model")
    default_nlp_model: str | None = Field(None, description="Default NLP model")
    supported_embedding_models: list[str | type[ModelCapabilityEnum]] = Field(
        default_factory=list, description="Supported embedding models"
    )
    supported_reranking_models: list[str | type[ModelCapabilityEnum]] = Field(
        default_factory=list, description="Supported reranking models"
    )
    supported_nlp_models: list[str] = Field(
        default_factory=list, description="Supported NLP models"
    )

    # Native dimensions mapping
    native_dimensions: dict[str | type[ModelCapabilityEnum], int] = Field(
        default_factory=dict, description="Model -> native dimensions mapping"
    )
    native_context_length: dict[str | type[ModelCapabilityEnum], int] = Field(
        default_factory=dict, description="Model -> native context length mapping"
    )
