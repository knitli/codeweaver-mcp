...
class TestIntentServiceBridge:
    ...
    async def test_initialization_success(
        self, intent_bridge, mock_intent_orchestrator, mock_auto_indexing_service
    ):
    ...
    async def test_initialization_missing_orchestrator(self, intent_bridge):
        ...
        with pytest.raises(
            ServiceIntegrationError, match="IntentOrchestrator service not available"
        ):
            await intent_bridge._initialize_provider()

    async def test_initialization_missing_auto_indexing(
        self, intent_bridge, mock_intent_orchestrator
    ):
    ...
    async def test_health_check_unhealthy_orchestrator(
        self, intent_bridge, mock_intent_orchestrator
    ):
    ...
    async def test_inject_services_into_context(
        self, intent_bridge, mock_intent_orchestrator, mock_auto_indexing_service
    ):
    ...
    async def test_trigger_background_indexing_success(
        self, intent_bridge, mock_auto_indexing_service
    ):
