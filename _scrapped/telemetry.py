...
from codeweaver.middleware import TelemetryService
...
class TelemetryMiddleware(Middleware):
    ...
    async def on_call_tool(
        ...
        method = getattr(context.message, "name", "unknown")
        ...
        try:
            ...
            await self._track_operation_success(method, params, response, duration)
            ...
            params = getattr(context.message, "params", None)
            await self._track_operation_error(method, params, e, duration)
    ...
    async def _track_indexing_success(
        ...
        result_data: dict[str, Any] = cast(dict[str, Any], response) if isinstance(response, dict) else {}
        file_count: int = result_data.get("files_processed", 0)
        language_distribution: dict[str, int] = result_data.get("languages", {})
    ...
    async def _track_search_success(
        ...
        # Try to extract results from response
        result_data: dict[str, Any] = cast(dict[str, Any], response) if isinstance(response, dict) else {}
        results: list[Any] = result_data.get("results", [])
