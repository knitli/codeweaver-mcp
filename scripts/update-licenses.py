...
__version__ = '0.1.0'
CONTRIBUTORS_GROUP = Group('Contributors', default_parameter=Parameter(negative=()), help='Manage contributors for the license update.')
CONTRIBUTORS = Parameter('-c', '--contributor', consume_multiple=True, help='Name and email of the contributor(s) to add. May be provided multiple times, or as a json list.', negative=(), json_list=True)
INTERACTIVE = Parameter('-i', '--interactive', action='store_true', negative=(), help='Run the script in interactive mode, prompting for contributors.')
app = App(name='Thread License Updater', version=__version__, default_command='add', help='Update licenses for files in the repository using Reuse. Respects .gitignore.', help_format='rich', default_parameter=Parameter(negative=()), help_on_error=True)
...
def years() -> str:
    ...
    if (year := str(datetime.now(UTC).year)) and year != '2025':
        return f'2025-{year}'
    return '2025'
BASE_CMD = ['reuse', 'annotate', '--year', years(), '--copyright', 'Knitli Inc. <knitli@knit.li>', '--fallback-dot-license', '--merge-copyrights', '--skip-existing']
...
def get_staged_files() -> list[Path]:
    ...
    try:
        ...
        if not git_path:
            ...
            sys.exit(1)
        result = subprocess.run([git_path, 'diff', '--cached', '--name-only'], capture_output=True, text=True, check=True)
