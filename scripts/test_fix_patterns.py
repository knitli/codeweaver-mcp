...
class RuffPatternTester:
    ...
    def run_ruff_check(self, target: str | None=None) -> tuple[bool, str]:
        ...
        try:
            result = subprocess.run([shutil.which('ruff'), 'check', target, '--select=TRY401,G004,TRY300', '--no-fix'], capture_output=True, text=True, timeout=30)
    ...
    def run_fix_script(self, targets: list[str] | None=None) -> tuple[bool, str]:
        ...
        if targets is None:
            ...
            targets = [target if Path(target).is_absolute() else str(self.script_dir / target) for target in targets]
        try:
            result = subprocess.run([str(self.fix_script), *targets, '--debug'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=30, cwd=self.script_dir.parent)
    ...
    def analyze_changes(self, test_files: dict[str, str]) -> dict[str, dict]:
        ...
        for filename, original_content in test_files.items():
            ...
            if file_path.exists():
                ...
                changes[filename] = {'modified': current_content != original_content, 'original_lines': len(original_content.splitlines()), 'current_lines': len(current_content.splitlines()), 'content': current_content}
            else:
    ...
    def validate_fixes(self, changes: dict[str, dict]) -> dict[str, list[str]]:
        ...
        for filename, change_info in changes.items():
            ...
            if 'f"' in content or "f'" in content:
                ...
                issues.extend((f'Line {i}: Possible remaining G004 violation: {line.strip()}' for i, line in enumerate(lines, 1) if ('logger.' in line or 'logging.' in line or 'log.' in line) and ('f"' in line or "f'" in line)))
            lines = content.splitlines()
            issues.extend((f'Line {i}: Possible remaining TRY401 violation: {line.strip()}' for i, line in enumerate(lines, 1) if '.exception(' in line and (', e)' in line or ', exc)' in line or ', exception)' in line)))
            in_try = False
    ...
    def run_comprehensive_test(self) -> bool:
        ...
        if initial_success:
            print('⚠️  No initial violations found - this might indicate a problem with test generation')
        else:
            violation_count = initial_output.count('TRY401') + initial_output.count('G004') + initial_output.count('TRY300')
        ...
        if final_success:
            ...
            remaining_violations = final_output.count('TRY401') + final_output.count('G004') + final_output.count('TRY300')
