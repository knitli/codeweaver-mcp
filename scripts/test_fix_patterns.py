...
'''
        ...
        test_files["try401_exceptions.py"] = '''
import logging
...
'''
        ...
        # Mixed violations
        test_files["mixed_violations.py"] = '''
import logging
...
'''
    ...
    def run_ruff_check(self, target: str = None) -> Tuple[bool, str]:
        """Run ruff check on target and return (success, output)."""
        target = target or str(self.test_dir)
        try:
            result = subprocess.run(
                ["ruff", "check", target, "--select=TRY401,G004,TRY300", "--no-fix"],
                capture_output=True,
                text=True,
                timeout=30
            )
    ...
    def run_fix_script(self, targets: List[str] = None) -> Tuple[bool, str]:
        ...
        if targets is None:
            ...
            targets = [str(self.script_dir / target) if not Path(target).is_absolute() else target for target in targets]
    ...
    def analyze_changes(self, test_files: Dict[str, str]) -> Dict[str, Dict]:
        ...
        for filename in test_files.keys():
            ...
            if file_path.exists():
                ...
                original_content = test_files[filename]
        ...
        for filename, change_info in changes.items():
            ...
            if "f\"" in content or "f'" in content:
                ...
                for i, line in enumerate(lines, 1):
                    if ("logger." in line or "logging." in line or "log." in line) and ("f\"" in line or "f'" in line):
                        issues.append(f"Line {i}: Possible remaining G004 violation: {line.strip()}")
            
            # Check for remaining TRY401 violations
            lines = content.splitlines()
            for i, line in enumerate(lines, 1):
                if ".exception(" in line and (", e)" in line or ", exc)" in line or ", exception)" in line):
                    issues.append(f"Line {i}: Possible remaining TRY401 violation: {line.strip()}")
